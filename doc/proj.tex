...
\section{Use cases} 
\subsection{Pharmacists and technicians}
Pharmacists and pharmacy technicians have different roles, but the line between them continue to blur in the duties they execute.  The extend of this blurring depends on the site.  On traditional sites, technicians are only concerned with supply, where as other hospitals have pharmacy technicians who carry out preliminary clinical checks and take drug histories from patients.
While in this system their roles are essentially identical in this system, a mechanism for changing access to functionality in the site will be made available, partially by making pharmacists and technicians different types of user.
...
When checking these details, consideration must be made of local policies with regard to recommended and available treatments.  
...
Dispensary users need to interract with the ward and patients concerning supply issues.  As such, dispensary pharmacy staff need to be able to view following:
\begin{itemize} 
...
An adminstrator will need to administer users, wards and products.  To this end, user with administrator privelages will need to perform CRUD operations of the following:
\begin{itemize}
    \item products 
    \item wards 
    \item users
...
\paragraph{}
\subsection{Patient}: The patients recorded in this system are the focus of the system.  Most functions are performed on a patient or its associated entities.
Properties:
\begin{itemize}
